...
auth.onAuthStateChanged(user => {
    ...
    if (user) { currentUser = user; loginScreen.style.display = 'none'; loadFromFirebase(); }
    else { currentUser = null; loginScreen.style.display = 'flex'; if (dbListenerUnsubscribe) dbListenerUnsubscribe(); }
});
...
async function loginFirebase() {
    ...
    try { await auth.signInWithEmailAndPassword(email, pass); errObj.style.display = 'none'; }
    catch (error) { errObj.innerText = "Erro: E-mail ou senha incorretos."; errObj.style.display = 'block'; }
}
...
async function registerFirebase() {
    ...
    const errObj = document.getElementById('loginError');
    if (!email || !pass) { errObj.innerText = "Preencha e-mail e senha."; errObj.style.display = 'block'; return; }
    try { await auth.createUserWithEmailAndPassword(email, pass); errObj.style.display = 'none'; await showSysAlert("Conta criada com sucesso!"); }
    catch (error) { errObj.innerText = "Erro: " + error.message; errObj.style.display = 'block'; }
}

function logoutFirebase() { if (dbListenerUnsubscribe) dbListenerUnsubscribe(); auth.signOut().then(() => window.location.reload()); }
...
const defaultTags = [
    { name: "💻 Desenvolvimento", color: "#3b82f6" }, { name: "🐛 Bug Fix", color: "#ef4444" },
    { name: "🧪 Teste", color: "#10b981" }, { name: "📅 Reunião", color: "#8b5cf6" },
    { name: "🚀 Deploy", color: "#f59e0b" }, { name: "🎨 Design", color: "#ec4899" }
];
...
// NOVO: Array global para segurar os responsáveis dentro do modal antes de salvar
...
function showSysModal(title, message, type = 'alert', placeholder = '') {
    return new Promise((resolve) => {
        ...
        titleEl.innerText = title; msgEl.innerText = message; inputEl.value = '';
...
async function showSysAlert(msg) { return await showSysModal('Aviso', msg, 'alert'); }
async function showSysConfirm(msg, tit) { return await showSysModal(tit, msg, 'confirm'); }
async function showSysPrompt(tit, p) { return await showSysModal(tit, '', 'prompt', p); }
        ...
        dbListenerUnsubscribe = db.collection('boards').where('members', 'array-contains', currentUser.email)
            .onSnapshot(snapshot => {
                ...
                snapshot.forEach(doc => {
                    const data = doc.data();
                    let parsedTags = (data.tags || defaultTags).map(t => {
                        let obj = typeof t === 'string' ? { name: t, color: '#3b82f6' } : { ...t };
                        if (obj.name.includes('Treino')) { obj.name = '🧪 Teste'; obj.color = '#10b981'; }
                        return obj;
                    });
                    newBoards.push({ id: doc.id, title: data.title, owner: data.owner });
                    newAllBoardsData[doc.id] = { tasks: data.tasks_string ? JSON.parse(data.tasks_string) : [], columns: data.columns_string ? JSON.parse(data.columns_string) : [], members: data.members || [currentUser.email], tags: parsedTags, owner: data.owner };
                });
                boards = newBoards;
                if (boards.length === 0) { if (!document.getElementById('sysOverlay').classList.contains('active') && !document.getElementById('templateOverlay').classList.contains('active')) openTemplateModal(); return; }
                renderBoardsList();
                if (!currentBoardId || !newAllBoardsData[currentBoardId]) { allBoardsData = newAllBoardsData; if (boards.length > 0) loadBoardData(boards[0].id); }
                else if (!isDragging && !isSavingLocally) {
                    const activeData = newAllBoardsData[currentBoardId];
                    allBoardsData = newAllBoardsData;
                    if (JSON.stringify(activeData.tasks) !== JSON.stringify(tasks) || JSON.stringify(activeData.columns) !== JSON.stringify(columns)) { tasks = activeData.tasks; columns = activeData.columns; tags = activeData.tags; updateTagsDropdown(); render(); }
                }
            });
...
function loadBoardData(boardId) {
    ...
    if (!bData) return;
    tasks = bData.tasks || []; columns = bData.columns || [{ id: 'todo', title: 'Pendências' }, { id: 'done', title: 'Concluído' }];

    // Suporte ao sistema legado: Se existir "assignee" antigo, converte para array "assignees"
    tasks.forEach(t => { if (!t.assignees) { t.assignees = t.assignee ? [t.assignee] : []; } });

    tags = bData.tags || [...defaultTags]; currentBoardMembers = bData.members || [currentUser.email];
    ...
    renderBoardsList(); updateTagsDropdown(); render();
}
...
function syncToFirebase() {
    ...
    syncTimeout = setTimeout(() => {
        ...
        }, { merge: true }).then(() => { isSavingLocally = false; syncTimeout = null; }).catch(err => { isSavingLocally = false; });
    }, 500);
}
...
async function applyTemplate(type) {
    ...
    if (!title) { if (boards.length === 0) openTemplateModal(); return; }
    let initCols = type === 'sprint' ? [{ id: 'col-1', title: 'Backlog' }, { id: 'col-2', title: 'Em Dev' }, { id: 'col-3', title: 'Code Review' }, { id: 'col-4', title: 'Deploy' }] :
        type === 'evento' ? [{ id: 'col-1', title: 'Planejamento' }, { id: 'col-2', title: 'Pendências' }, { id: 'col-3', title: 'Execução' }, { id: 'col-4', title: 'Finalizado' }] :
            [{ id: 'col-1', title: 'A Fazer' }, { id: 'col-2', title: 'Fazendo' }, { id: 'col-3', title: 'Concluído' }];
    const newId = 'board-' + Date.now();
    await db.collection('boards').doc(newId).set({ title, owner: currentUser.email, members: [currentUser.email], tasks_string: "[]", columns_string: JSON.stringify(initCols), tags: [...defaultTags] });
    currentBoardId = newId;
}
function switchBoard(boardId) { syncToFirebase(); loadBoardData(boardId); }
async function deleteCurrentBoard() {
    if (allBoardsData[currentBoardId].owner !== currentUser.email) return showSysAlert("Apenas o dono pode excluir.");
    if (boards.length <= 1) return showSysAlert("Não pode excluir o único quadro.");
    if (await showSysConfirm("Tem certeza? Isso apagará o quadro para todos.")) { const id = currentBoardId; currentBoardId = boards.find(b => b.id !== id).id; await db.collection('boards').doc(id).delete(); }
}
...
function render() {
    ...
    const board = document.getElementById('boardMain'); board.innerHTML = '';
    ...
    const filteredTasks = tasks.filter(t => {
        ...
        return matchTag && matchMyTasks && (term === '' || t.text.toLowerCase().includes(term));
    });

    columns.forEach(col => {
        const columnEl = document.createElement('div'); columnEl.className = 'column'; columnEl.id = col.id;
        columnEl.innerHTML = `<div class="column-header"><div style="display:flex; align-items:center;"><span contenteditable="true" class="column-title-edit" onblur="updateColumnTitle('${col.id}', this.innerText)">${col.title}</span><button class="btn-add-rounded" onclick="openModal(null, '${col.id}')" style="margin-left: 10px;">+</button></div><div style="display:flex; align-items:center;"><span class="count-badge">${filteredTasks.filter(t => t.status === col.id).length}</span><button class="column-delete-btn" onclick="deleteColumn('${col.id}')">✖</button></div></div><div class="tasks-container" data-status="${col.id}"></div>`;
        board.appendChild(columnEl);
        const container = columnEl.querySelector('.tasks-container');

        filteredTasks.filter(t => t.status === col.id).forEach(t => {
            const isDone = col.id === 'done' || col.title.toLowerCase().includes('conclu');
            const card = document.createElement('div'); card.className = `card ${isDone ? 'finalizado' : ''}`; card.id = t.id; card.onclick = (e) => { if (!e.target.classList.contains('card-cover')) openModal(t.id); };
            ...
            card.innerHTML = `${t.cover ? `<img src="${t.cover}" class="card-cover" onclick="openImageViewer(event, this.src)">` : ''}<div class="tag-row"><div class="tag" style="background:${tagObj.color}">${tagObj.name}</div>${t.endDate ? `<div class="date-display ${(t.endDate < getTodayString() && !isDone) ? 'overdue' : ''}">${(t.endDate < getTodayString() && !isDone) ? '⚠️' : '📅'} ${formatDate(t.endDate)}</div>` : ''}</div><span class="card-text">${t.text}</span>${t.subtasks && t.subtasks.length > 0 ? `<div style="font-size: 10px; color: var(--text-sub);">Checklist ${t.subtasks.filter(s => s.done).length}/${t.subtasks.length}</div>` : ''}<div class="card-footer"><div class="prio-badge prio-${t.priority || 'Média'}">${t.priority || 'Média'}</div>${avatarsHtml}</div>`;
            container.appendChild(card);
        });
    });
    setupCardDragAndDrop(); setupColumnDragAndDrop();
}
    ...
    document.querySelectorAll('.tasks-container').forEach(container => {
        new Sortable(container, {
            ...
            onEnd: function (evt) {
                ...
                if (task) {
                    if (task.status !== newStatus) { if (!task.history) task.history = []; task.history.unshift({ date: new Date().toISOString(), user: currentUser.email, action: `Moveu para "${columns.find(c => c.id === newStatus)?.title}"` }); }
...
function setupColumnDragAndDrop() {
    new Sortable(document.getElementById('boardMain'), {
        ...
        onEnd: function () {
            isDragging = false;
            let n = []; document.querySelectorAll('.column').forEach(e => { let c = columns.find(x => x.id === e.id); if (c) n.push(c); });
...
// --- LOGICA DE SELEÇÃO MULTIPLA NO MODAL ---
...
function renderAssigneeChips() {
    ...
    container.innerHTML = modalSelectedAssignees.map(m =>
        `<div class="assignee-chip" onclick="removeAssigneeFromModal('${m}')" title="Clique para remover">${m.split('@')[0]} <span>&times;</span></div>`
    ).join('');
}
...
function openModal(taskId = null, initialStatus = null) {
    ...
    document.getElementById('modalDateStart').value = ''; document.getElementById('modalDateEnd').value = '';
    ...
    if (taskId) {
        ...
        if (!task) return;
        document.getElementById('modalTitle').innerText = "Detalhes da Tarefa"; document.getElementById('modalSaveBtn').innerText = "Atualizar Tarefa"; document.getElementById('modalDeleteBtn').style.display = 'block';
        document.getElementById('modalTaskInput').value = task.text; document.getElementById('modalDescriptionInput').value = task.description || '';
        document.getElementById('modalTagInput').value = task.tag?.name || task.tag || ''; updateColorPicker();
        document.getElementById('modalPriorityInput').value = task.priority || 'Média';
        document.getElementById('modalDateStart').value = task.startDate || ''; document.getElementById('modalDateEnd').value = task.endDate || '';
        ...
        editingTaskId = null; document.getElementById('modalTitle').innerText = "Nova Tarefa"; document.getElementById('modalSaveBtn').innerText = "Criar Tarefa"; document.getElementById('modalDeleteBtn').style.display = 'none';
        tempSubtasks = []; tempComments = []; tempHistory = []; document.getElementById('modalDateStart').value = getTodayString();
        modalSelectedAssignees = [];
        if (tags.length > 0) { document.getElementById('modalTagInput').selectedIndex = 0; updateColorPicker(); }
    }

    renderAssigneeSelect();
    renderAssigneeChips();
    renderSubtasksList(); renderCommentsList(); renderHistory(); switchDescTab('preview'); switchBottomTab('comments'); modal.classList.add('active');
}
...
async function saveTaskBtnClick() {
    ...
    let newTaskData = {
        text: text, description: document.getElementById('modalDescriptionInput').value,
        tag: tagObj, priority: document.getElementById('modalPriorityInput').value,
        assignees: modalSelectedAssignees, cover: document.getElementById('modalCoverInput').value,
        startDate: document.getElementById('modalDateStart').value, endDate: document.getElementById('modalDateEnd').value,
        subtasks: tempSubtasks, comments: tempComments, history: tempHistory
    };
    ...
    if (editingTaskId) {
        ...
        if (i > -1) {
            ...
            if (JSON.stringify(oldAssignees.sort()) !== JSON.stringify(newTaskData.assignees.sort())) addHistoryLog(`Atualizou os responsáveis`);
            tasks[i] = { ...tasks[i], ...newTaskData };
        }
    } else {
        newTaskData.id = 'id-' + Date.now(); newTaskData.status = currentTargetColumn || (columns.length > 0 ? columns[0].id : 'todo'); newTaskData.history = [{ date: new Date().toISOString(), user: currentUser.email, action: 'Criou a tarefa' }]; tasks.push(newTaskData);
    }
    save(); closeModal();
}

async function deleteTaskFromModal() { if (!editingTaskId) return; if (await showSysConfirm("Excluir tarefa permanentemente?")) { tasks = tasks.filter(t => t.id !== editingTaskId); save(); closeModal(); } }
function updateTagsDropdown() { const m = document.getElementById('modalTagInput'); const f = document.getElementById('filterTag'); const currVal = m.value; m.innerHTML = tags.map(t => `<option value="${t.name}">${t.name}</option>`).join(''); f.innerHTML = `<option value="all">🏷️ Todas as Tags</option>` + tags.map(t => `<option value="${t.name}">${t.name}</option>`).join(''); if (currVal && tags.find(t => t.name === currVal)) m.value = currVal; f.value = currentTagFilter; updateColorPicker(); }
function updateColorPicker() { const selectedTagName = document.getElementById('modalTagInput').value; const tag = tags.find(t => t.name === selectedTagName); if (tag && tag.color) document.getElementById('newTagColor').value = tag.color; }
function updateCurrentTagColor() { const selectedTagName = document.getElementById('modalTagInput').value; const newColor = document.getElementById('newTagColor').value; const tagIndex = tags.findIndex(t => t.name === selectedTagName); if (tagIndex > -1) { tags[tagIndex].color = newColor; tasks.forEach(t => { if (t.tag && t.tag.name === selectedTagName) t.tag.color = newColor; else if (typeof t.tag === 'string' && t.tag === selectedTagName) t.tag = { name: selectedTagName, color: newColor }; }); syncToFirebase(); render(); } }
async function addNewTag() { const tName = await showSysPrompt("Nome da Nova Tag:"); if (tName) { const tColor = document.getElementById('newTagColor').value || '#3b82f6'; if (!tags.find(x => x.name === tName)) { tags.push({ name: tName, color: tColor }); updateTagsDropdown(); document.getElementById('modalTagInput').value = tName; updateColorPicker(); syncToFirebase(); } } }
function switchBottomTab(tab) { document.getElementById('btnTabComments').classList.remove('active'); document.getElementById('btnTabHistory').classList.remove('active'); document.getElementById('tabComments').style.display = 'none'; document.getElementById('tabHistory').style.display = 'none'; if (tab === 'comments') { document.getElementById('btnTabComments').classList.add('active'); document.getElementById('tabComments').style.display = 'block'; } else { document.getElementById('btnTabHistory').classList.add('active'); document.getElementById('tabHistory').style.display = 'block'; } }
function addHistoryLog(action) { tempHistory.unshift({ date: new Date().toISOString(), user: currentUser.email, action: action }); }
function renderHistory() { const container = document.getElementById('historyList'); if (tempHistory.length === 0) { container.innerHTML = '<div style="color:var(--text-sub); font-size:0.9rem; text-align:center; padding: 20px;">Nenhum registro de histórico.</div>'; return; } container.innerHTML = tempHistory.map(h => { const d = new Date(h.date); return `<div class="history-item"><b>${h.user.split('@')[0]}</b> ${h.action} <span style="font-size:0.75rem; opacity:0.6; margin-left:5px;">(${d.toLocaleDateString()} ${d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })})</span></div>`; }).join(''); }
function addComment() { const input = document.getElementById('commentInput'); if (input.value.trim()) { const now = new Date(); tempComments.push({ text: input.value.trim(), author: currentUser.email, date: now.toLocaleDateString() + ' ' + now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }); input.value = ''; renderCommentsList(); addHistoryLog("Adicionou um comentário"); } }
function renderCommentsList() { const container = document.getElementById('commentsList'); if (tempComments.length === 0) { container.innerHTML = '<div style="color:var(--text-sub); font-size:0.9rem; text-align:center; padding: 20px;">Nenhum comentário ainda.</div>'; return; } container.innerHTML = tempComments.map(c => { let text = c.text.replace(/(@\S+)/g, '<span class="mention">$1</span>'); return `<div class="comment-item"><div class="comment-header"><span>${c.author.split('@')[0]}</span><span>${c.date}</span></div><div class="comment-text">${text}</div></div>`; }).join(''); container.scrollTop = container.scrollHeight; }
function save() { render(); syncToFirebase(); }
function saveColumns() { render(); syncToFirebase(); }
async function deleteColumn(id) { if (tasks.filter(t => t.status === id).length > 0) return showSysAlert("Esta coluna contém tarefas. Mova-as antes de excluir."); const ok = await showSysConfirm("Excluir coluna vazia?"); if (ok) { columns = columns.filter(c => c.id !== id); saveColumns(); } }
...
function applyFilters() { currentTagFilter = document.getElementById('filterTag').value; searchTerm = document.getElementById('searchInput').value.toLowerCase(); render(); }
...
function switchDescTab(mode) { if (mode === 'write') { document.getElementById('btnWrite').classList.add('active'); document.getElementById('btnPreview').classList.remove('active'); document.getElementById('modalDescriptionInput').style.display = 'block'; document.getElementById('descPreview').style.display = 'none'; } else { document.getElementById('btnWrite').classList.remove('active'); document.getElementById('btnPreview').classList.add('active'); document.getElementById('modalDescriptionInput').style.display = 'none'; document.getElementById('descPreview').style.display = 'block'; document.getElementById('descPreview').innerHTML = simpleMarkdown(document.getElementById('modalDescriptionInput').value); } }
...
function toggleView() { isCalendarView = !isCalendarView; document.getElementById('btnViewToggle').style.background = isCalendarView ? 'var(--accent)' : 'transparent'; document.getElementById('btnViewToggle').style.color = isCalendarView ? 'white' : 'var(--text-sub)'; document.getElementById('btnAddColBtn').style.display = isCalendarView ? 'none' : 'block'; render(); }
function renderCalendar() { document.getElementById('boardMain').style.display = 'none'; document.getElementById('calendarMain').style.display = 'flex'; const head = document.getElementById('calendarHeader'); const grid = document.getElementById('calendarGrid'); const monthNames = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]; head.innerHTML = `<h2 style="margin:0;">${monthNames[currentMonth]} ${currentYear}</h2><div style="display:flex; gap:10px;"><button class="btn-secondary" onclick="changeMonth(-1)">⬅️</button><button class="btn-secondary" onclick="currentMonth=new Date().getMonth(); currentYear=new Date().getFullYear(); renderCalendar();">Hoje</button><button class="btn-secondary" onclick="changeMonth(1)">➡️</button></div>`; grid.innerHTML = ''; const daysInMonth = new Date(currentYear, currentMonth + 1, 0).getDate(); const firstDay = new Date(currentYear, currentMonth, 1).getDay(); for (let i = 0; i < firstDay; i++) grid.innerHTML += `<div class="calendar-day empty"></div>`; for (let d = 1; d <= daysInMonth; d++) { let dayStr = `${currentYear}-${(currentMonth + 1).toString().padStart(2, '0')}-${d.toString().padStart(2, '0')}`; let html = tasks.filter(t => t.endDate === dayStr || t.startDate === dayStr).map(t => `<div class="cal-task-pill" onclick="openModal('${t.id}')">${t.startDate === dayStr ? '🟢' : '🔴'} ${t.text}</div>`).join(''); grid.innerHTML += `<div class="calendar-day"><div class="calendar-day-header">${d}</div>${html}</div>`; } }
...
function addCollaborator() { if (allBoardsData[currentBoardId].owner !== currentUser.email) return showSysAlert("Apenas o dono pode adicionar."); let e = document.getElementById('collabEmail').value.trim().toLowerCase(); if (e && !currentBoardMembers.includes(e)) { currentBoardMembers.push(e); document.getElementById('collabEmail').value = ''; syncToFirebase(); renderMembersList(); showSysAlert(`O quadro aparecerá para ${e}`); } }
async function removeCollaborator(e) { if (allBoardsData[currentBoardId].owner !== currentUser.email) return showSysAlert("Apenas o dono pode remover."); if (e !== currentUser.email && await showSysConfirm(`Remover ${e}?`)) { currentBoardMembers = currentBoardMembers.filter(m => m !== e); syncToFirebase(); renderMembersList(); } }
...
function renderCharts() {
    ...
    tagsChartInstance = new Chart(document.getElementById('tagsChart'), { type: 'doughnut', data: { labels: Object.keys(tagCounts), datasets: [{ data: Object.values(tagCounts), backgroundColor: tags.map(t => t.color), borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { color: textColor, font: { size: 11 } } } } } });
    if (statusChartInstance) statusChartInstance.destroy();
    statusChartInstance = new Chart(document.getElementById('statusChart'), { type: 'bar', data: { labels: columns.map(c => c.title), datasets: [{ label: 'Tarefas', data: columns.map(c => tasks.filter(t => t.status === c.id).length), backgroundColor: '#3b82f6', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { color: textColor } }, x: { ticks: { color: textColor } } }, plugins: { legend: { display: false } } } });
    if (priorityChartInstance) priorityChartInstance.destroy();
    priorityChartInstance = new Chart(document.getElementById('priorityChart'), { type: 'polarArea', data: { labels: ['Alta', 'Média', 'Baixa'], datasets: [{ data: [stats.priority.Alta, stats.priority.Média, stats.priority.Baixa], backgroundColor: ['rgba(239, 68, 68, 0.7)', 'rgba(245, 158, 11, 0.7)', 'rgba(59, 130, 246, 0.7)'] }] }, options: { responsive: true, maintainAspectRatio: false, scales: { r: { grid: { color: 'rgba(128,128,128,0.2)' }, ticks: { display: false } } }, plugins: { legend: { position: 'right', labels: { color: textColor, font: { size: 11 } } } } } });
    ...
    assigneeChartInstance = new Chart(document.getElementById('assigneeChart'), {
        type: 'bar',
        data: { labels: assignLabels.map(l => l.split('@')[0]), datasets: [{ label: 'Total de Tarefas', data: assignTotal, backgroundColor: '#6366f1', borderRadius: 4 }, { label: 'Entregues (Concluído)', data: assignDone, backgroundColor: '#10b981', borderRadius: 4 }] },
        options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { color: textColor } }, x: { ticks: { color: textColor } } }, plugins: { legend: { position: 'top', labels: { color: textColor } } } }
    });
}
