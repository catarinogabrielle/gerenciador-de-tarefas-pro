...
function render() {
    ...
    columns.forEach(col => {
        ...
        columnEl.className = 'column'; columnEl.id = col.id;
...
function openModal(taskId = null, initialStatus = null) {
    ...
    switchDescTab('preview'); switchBottomTab('comments');
...
async function saveTaskBtnClick() {
    ...
    let newTaskData = {
        ...
        assignee: document.getElementById('modalAssigneeInput').value,
...
function addHistoryLog(action) {
    tempHistory.unshift({ date: new Date().toISOString(), user: currentUser.email, action: action });
}
...
function addComment() {
    ...
    if (input.value.trim()) {
        ...
        input.value = ''; renderCommentsList(); addHistoryLog("Adicionou um comentário");
    }
}
